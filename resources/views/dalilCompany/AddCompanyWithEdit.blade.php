...
<form method="POST" action="{{ route('storeCompanyForUser') }}" class=" m-5  " enctype="multipart/form-data" style="box-shadow: rgba(9, 30, 66, 0.25) 0px 4px 8px -2px, rgba(9, 30, 66, 0.08) 0px 0px 0px 1px !important;background-color:#fff;padding: 2rem;">
    @csrf
    {{-- ارسال الأخطاء --}}
     ...
     @isset($customizeUsers->country)
        <div class="col-md-6 mb-3">
            ...
            <select name="name_country" class="form-select" id="ArabCountry">
                ...
                @isset($countriess)
                @foreach($countriess as $countryy)
                <option value="{{ $countryy->country_name }}" @isset($Company) @if($Company->name_country==$countryy->country_name) selected @endif @endisset>{{ $countryy->country_name }}</option>
                @endforeach
                @endisset
              </select>
        </div>
